...
def f06(string1, string2):
    ...
    if len(string1) == len(string2):
        ...
        print "'{0}' is longer than '{1}' by {2} chars".format(longer, shorter, diff)

    percentage = 100.0 * len(shorter) / len(longer)
    print "'{0}' has only {1}% the number of characters of '{2}'".format(shorter, round(percentage, 2), longer)
...
###############################################################################
def f09():
    multiples = [x for x in range(1,500) if (x % 3 == 0 or x % 5 == 0)]
    all_sum = sum(multiples)
    return all_sum
...
def f12():
    ...
    with open('log_file.txt', 'w') as fout:
        while user_input != "done" and user_input != "\"done\"":
            user_input = raw_input("Input a number > ")
...
# (6) Add an exclamation mark to Python.
